...
import { defineRecord, defineSchemaUpdate, defineUnion, nextSchema } from "@palantir/pack.schema";
...
describe("generateScopeFromSchema", () => {
  ...
  it("should generate setCollectionRecord overloads for union models referencing changed records", () => {
    ...
    // Union model should have setCollectionRecord overload (for full-type creation)
    expect(result).toContain(
  ...
  it("should generate generic fallback for models with only additive changes", () => {
    ...
    expect(result).not.toContain(
      "updateRecord(ref: RecordRef<typeof LabelModel>, data: LabelUpdate_v2): void;",
    );
