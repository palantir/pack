 ...
 * Fields with defaults are required (not optional) in read schemas.
 */
  ...
  for (const [exportName, item] of Object.entries(schema)) {
    ...
    for (const [fieldName, fieldType] of Object.entries(item.fields)) {
      ...
      if (fieldType.type === TypeKind.OPTIONAL && hasDefault) {
        // Field has default: unwrap Optional, make required in Zod
        const innerZod = fieldType.item
          ? convertFieldTypeToZodSchema(fieldType.item, schema, version)
          : "z.unknown()";
        fieldLines.push(`  ${fieldName}: ${innerZod}`);
      } else if (fieldType.type === TypeKind.OPTIONAL) {
        const innerZod = fieldType.item
          ? convertFieldTypeToZodSchema(fieldType.item, schema, version)
          : "z.unknown()";
...
/**
 * Generate per-version Zod schemas with `.passthrough()` from a schema with version chain.
 *
 ...
 * @param typeImportPath - Import path for the per-version type definitions (e.g., "./types_v1.js")
