...
import { createInMemoryDocumentServiceConfig } from "@palantir/pack.state.core";
...
export function getDocumentServiceConfig(
  ...
  return config.isTestMode
    ? createInMemoryDocumentServiceConfig({ autoCreateDocuments: true })
    : createFoundryDocumentServiceConfig();
}
