...
/**
 * Customizes the human-facing output of {@link createProject} so that each CLI
 * that reuses this engine can print its own branding without duplicating the
 * generation flow.
 */
export interface CreateProjectMessaging {
  /** Noun used in status lines, e.g. "SDK project" or "project". Defaults to "project". */
  readonly entity?: string;
  /**
   * Returns the lines printed under "Next steps". When omitted, a generic
   * `cd` / `npm install` / `npm run build` sequence is printed.
   */
  readonly nextSteps?: (
    args: { readonly projectName: string; readonly skipInstall?: boolean },
  ) => readonly string[];
}

export interface CreateProjectOptions extends CreateCommandOptions {
  /** Overrides the default status/next-step messaging. */
  readonly messaging?: CreateProjectMessaging;
}

function defaultNextSteps(
  { projectName, skipInstall }: { projectName: string; skipInstall?: boolean },
): readonly string[] {
...
/**
 * Scaffolds a project from a template using the shared EJS generation engine.
 *
 * `options.template` must be a resolvable template reference: an absolute/relative
 * local path, an installed npm package name, or `"default"`. Callers that ship
 * built-in templates should resolve them to absolute paths before calling this.
 */
export async function createProject(
  ...
  const entity = options.messaging?.entity ?? "project";

  try {
    logger.info(`Creating ${entity}: ${projectName}`);
