...
describe("Foundry Document Status Tracking", () => {
  ...
  describe("metadata channel liveness", () => {
    ...
    it("should report CONNECTING on the data channel while metadata is being loaded", async () => {
      const docRef = createDocRef(mockApp, "test-doc-live-7" as DocumentId, testSchema);

      // startDocumentSync only reports liveness once the metadata round-trip completes, so
      // without an up-front CONNECTING the channel reads DISCONNECTED while it is opening.
      const unsubscribe = service.onStateChange(docRef, () => {});

      expect(service.getDocumentStatus(docRef).data.live).toBe(DocumentLiveStatus.CONNECTING);

      await vi.runAllTimersAsync();
      unsubscribe();
    });

    it("should report ERROR on the data channel when metadata loading fails", async () => {
      vi.mocked(Documents.get).mockRejectedValue(new Error("metadata unavailable"));

      const docRef = createDocRef(mockApp, "test-doc-live-8" as DocumentId, testSchema);

      const unsubscribe = service.onStateChange(docRef, () => {});
      await vi.runAllTimersAsync();

      const status = service.getDocumentStatus(docRef);
      expect(status.data.load).toBe(DocumentLoadStatus.ERROR);
      // A failed open is not a closed channel; leaving live at DISCONNECTED would be
      // indistinguishable from a deliberate teardown.
      expect(status.data.live).toBe(DocumentLiveStatus.ERROR);

      unsubscribe();
    });

    it("should clear a subscription error when the metadata channel is closed deliberately", async () => {
      mockEventService.subscribeToMetadataUpdates.mockRejectedValueOnce(
        new Error("window is not defined"),
      );

      const docRef = createDocRef(mockApp, "test-doc-live-6" as DocumentId, testSchema);

      const unsubscribe = service.onMetadataChange(docRef, () => {});
      await vi.runAllTimersAsync();
      expect(service.getDocumentStatus(docRef).metadata.error).toBeDefined();

      // Closing on purpose is not a failure, so the error must not outlive the channel — it
      // would otherwise still be attached through the next reopen's CONNECTING window.
      unsubscribe();
