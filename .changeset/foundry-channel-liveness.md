...
Two known limitations, both follow-up work:

`live` tracks subscription lifecycle, not transport health. `EventServiceCometD` listens only on `/meta/handshake` and silently resubscribes, so nothing drives `live` back down when the websocket drops — a channel that reached `CONNECTED` keeps reporting it through an outage. `CONNECTED` therefore means "the subscription was established", not "the connection is up right now". Surfacing transport state (a `/meta/connect` listener feeding a connection-state hook) applies equally to the activity and presence channels.

`InMemoryDocumentService` still reports `DISCONNECTED` on every channel, because it sets `load` only. Consumers wiring it should not bind indicators to `live`.
